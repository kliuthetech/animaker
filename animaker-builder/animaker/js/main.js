...
function reset_timeout(t=60000) {
...
$.fn.transition_to = function(to, time=1000) {
	...
	if ($(this).hasClass('active'))
		this.removeClass('active').animate({opacity:0}, time, function(){
			$(this).addClass('inactive')
			to.removeClass('inactive').animate({opacity:1}, time, function(){
				$(this).addClass('active')
				if ($(this).hasClass('attract'))
					clearTimeout(timeout)
...
$(document).ready(function() {
	...
	$('.attract').on('click touchstart touchmove', function(){
		if (attract.hasClass('active'))
			attract.transition_to(intro)
	})


	$('.intro .button').on('click touchstart touchmove', function(){
		if (intro.hasClass('active'))
			intro.transition_to(gallery)
	})

	gallery.on('click mousemove touchstart', function(){reset_timeout()})
	$('.gallery .animals li img').on('click', function(){
	...
	$('.viewer .button').on('touchstart', function(){
